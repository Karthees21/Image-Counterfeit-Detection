...
def train_Ela_Model(csv_file , lr , ep):
    ...
    history = model.fit(X_train, Y_train, batch_size=batch_size1, epochs=epochs1, validation_data=(X_val, Y_val), verbose=2)
    print(history)
    ...
    legend2 = ax[1].legend(loc='best', shadow=True)


    def plot_confusion_matrix(cm_, classes, normalize=True, title_='Confusion matrix', cmap=cm.get_cmap("Spectral")):
        """
        ...
        for i, j in itertools.product(range(cm_.shape[0]), range(cm_.shape[1])):
            plt.text(j, i, cm_[i, j],horizontalalignment="center",color="white" if cm_[i, j] > thresh else "black")
